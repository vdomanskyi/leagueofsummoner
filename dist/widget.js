...
var matches_default = async (assets2, { user, character, matchIds }, fields2, firstRender) => {
  ...
  if (firstRender) store_default.init(character, matches);
...
var createMatchList = (assets2, user) => {
  ...
  (storeData?.matches || []).forEach((match) => {
    const participant = getParticipant(user, match);
    if (participant) matches.push(createMatch(assets2, match.metadata.matchId, participant));
  });

...
const createAnimation = (pauseDuration) => {
  const defaults = {
    duration: 3,
    ease: 'power1.inOut'
  }

  const timeline = gsap.timeline({ repeat: -1, yoyo: false, delay: pauseDuration, repeatDelay: pauseDuration, defaults });

  timeline.to('.animation', { x: -200 });
  timeline.to('.animation', { x: -400 }, `+=${pauseDuration}`);
  timeline.to('.animation', { x: 0 }, `+=${pauseDuration}`)
}

const render = async (widget) => {
  const F = await frames();

  // widget.append([r._background, _animation.append(r._row)]);
};
...
window.addEventListener('onWidgetLoad', async (obj) => {
  ...
  apiKey = API_KEY;
  widget = $('.widget')
    .toggleClass('flip-content', flipContent === 'true')
    .append(createBorder(flipBorder === 'true'));
  ...
  // animationTimeline = createAnimation(pauseDuration);

  // setInterval(async () => {
  //   const r = await render();
  //   const currentX = gsap.getProperty('.animation', 'x');

  //   animationTimeline.kill();

  //   _widget.append([r._background, _animation.append(r._row)]);

  //   gsap.set('.animation', { x: currentX });
